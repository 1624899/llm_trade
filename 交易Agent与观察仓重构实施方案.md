...
新增一个独立的交易 Agent，给它一个持久模拟账户，初始资金 10000 元。账户只在首次初始化时注入资金，后续每次运行都沿用数据库中的现金、持仓、交易流水和盈亏状态，不会自动重置。
...
3. 获取宏观环境。
4. 对已有持仓调用 ExitAgent。
5. 调用 TradingAgent 生成交易决策。
6. TradingAccount 校验硬约束。
7. 执行模拟买入、卖出或持有。
8. 写入交易流水。
9. 输出交易报告。
