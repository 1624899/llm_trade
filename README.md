...
- 默认初始资金 16000 元，可在 `config/config.yaml` 的 `trading_account.initial_cash` 中修改；已创建账户也可以用 `python main.py --set-trade-cash 50000` 手动调整现金。
- 后续运行持续滚动现金、当前持仓、历史持仓、交易流水和盈亏。
- 卖出清仓后，历史持仓会保留已实现盈亏、卖出价和收益率，继续参与账户总盈亏统计。
- 交易仓最多同时持有 5 只股票。
- A 股默认按 100 股一手买入。
- 买入后至少持有 5 个交易日。
- 卖出后 5 个交易日内不回买同一股票。
...
11. `ExitAgent` 在盘后进行持仓诊断，`ReflectionAgent` 只对已清仓的交易仓亏损样本进行复盘。
...
```bash
python main.py --set-trade-cash 50000
python main.py --set-trade-cash 50000 --reset-trade-baseline
```

第一条只调整当前可用现金；第二条会同时把初始资金基准改为 50000，适合重新开始一轮模拟账户统计。也可以在首次建账前修改 `config/config.yaml`：
...
- 写入 `trade_orders`，并输出交易仓执行报告；历史清仓记录会继续展示在历史持仓区。
...
指定分析不会调用 `StockScreener`。如果综合结论明确为“强推荐”，且观察仓当前没有该股票，会自动补入观察仓；其他结论只生成报告，不自动加入。报告会写入：
...
`outputs/latest_targeted_analysis.md` 是指定股票的绝对诊断报告，不参与候选池相对排名。只有单独分析明确为“强推荐”的标的会自动加入观察仓；同一只股票可能在选股报告里属于“配置/轻仓验证”，但在单独分析里显示为中性、谨慎或等待买点，这种情况不会自动入仓观察。
...
- 更新观察仓和交易仓最新价格。
- 计算观察仓浮动收益和交易仓账户权益；交易仓的历史清仓记录会纳入已实现盈亏、总盈亏和交易反思。
- 调用宏观环境判断。
- 观察仓仍会通过 `ExitAgent` 评估当前持仓是否减仓或清仓；交易仓只对已结算历史持仓做清仓复盘，当前浮亏持仓只刷新市值和浮动盈亏。
- 仅对交易仓已清仓亏损案例生成反思并沉淀到规则本；观察仓和未清仓持仓只刷新浮盈浮亏，不写入反思规则。交易仓清仓记录会写入 `last_reflected_at` / `reflection_count`，避免每天重复反思同一笔亏损。
- 加仓同一只股票时不新增持仓行，而是按 `(原成本 × 原数量 + 加仓价 × 加仓数量) / 加仓后数量` 重算平均成本价。
- 减仓时不新建历史持仓行，仍保留在当前持仓中，展示“持有数量、已卖数量、未实现盈亏、已实现盈亏、持仓总盈亏”；完全清仓后才进入历史持仓。
...
- 可以通过按钮触发原来的 bash 命令，包括同步数据、补历史日线、派生周/月线、执行选股、走步回测、模拟调仓和交易反思。
...
- `trading_positions`：交易仓当前持仓与历史清仓记录；当前最多 5 只，历史记录保留已实现盈亏。
